...
def fileRename(request):
    if request.method == "POST":
        ...
        # graph = Logs.objects.get(file_id=fileId)
        # if graph:
...
def newFile(request):
    if request.method == "POST":
        ...
        # if count != 0:
        #     if updateUserData.add_files == plan.add_files/count:
        #         return Response({'error': 'You have reached your File limit'}, status=400)
...
def deleteGraph(request):
    if request.method == 'POST':
        ...
        # if graph_data.user_id!= user['user_id']:
        #     return Response({'error': 'You do not have the right to perform this action.'}, status=400)
        
        # Decrease the graph limit for the owner
